...
func (u User) Create(ctx context.Context, traceID string, nu NewUser, now time.Time) (Info, error) {
	...
	// collect everything you need before you build your value
	usr := Info{
		ID:           uuid.New().String(),
		Name:         nu.Name,
		Email:        nu.Email,
	...
	// make things easy to understand - according to Bill Kennedy - never use an ORM
	const q = `
	INSERT INTO users
...
func (u User) Update(ctx context.Context, traceID string, claims auth.Claims, userID string, uu UpdateUser, now time.Time) error {
	...
	// update everything - go after simple until you cant - add complexity only when you need it.
	const q = `
	UPDATE
		users
	SET 
