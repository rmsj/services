...
import (
	...
	"log"
	"net/http"
	_ "net/http/pprof" // there is a init function that bounds the routes we need (debug/pprof), but we don't need anything else
	"os"
	"os/signal"
	"syscall"
	"time"
...
TODO: You might want to reset your DB_HOST env var during test tear down.
TODO: Service should start even without a DB running yet.
TODO: symbols in profiles: https://github.com/golang/go/issues/23376 / https://github.com/google/pprof/pull/366
*/
...
func main() {
	...
	// we make code easy to understand, not easy to do
	if err := run(log); err != nil {
		// the unexported run function let us deal with all start up and shut down
...
func run(log *log.Logger) error {
	...
	// It'' blocking because there is no default case
	select {
	...
	case sig := <-shutdown:
		...
		// this is another blocking call - either all the requests completes, or we reach the timeout according the the config
